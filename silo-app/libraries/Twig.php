...
class Twig
{
  ...
  /**
	...
	private $functions_asis = [
		'base_url', 'site_url', 'current_url', 'cut_string'	
		];
...
//		'set_radio', 'set_select', 'set_checkbox',
	...
	public function __construct($params = [])
	{
	  ...
	  // addGlobal Variabel
		...
		$CI->load->model(array('blog_category', 'blog_pages', 'blog_post', 'blog_setting')); 
		
		$addGlobal = [
		       'site_name' => $CI->site->site('title'),
		       
		       'site_description' => $CI->site->site('description'),
		       
		       'site_keywords' => $CI->site->site('keywords'),
		
		       'list_category' => $CI->blog_category->allcategory(),
		       
		       'list_page' => $CI->blog_pages->allpages(),
		       
		       'populer_post' => $CI->blog_post->top(6),
		       'latest_post' => $CI->blog_post->listpost(0, 'publish', 8),
		       
		       'logged' => $CI->user->logged(),
		       'logged_email' => $CI->blog_setting->get_account()['email'],
	...
	public function addGlobal($data = array())
	{
		$this->createTwig();
		if (is_array($data))
		{
